# MercyShield ∞ Pure Thunder Eternal

**Divine Fortress Android Application**  
Coforged in absolute grandmasterism — combined Play Integrity + SafetyNet fallback + ultimate emulator/root/mock-location annihilation + live UI burst revelation + auto-shield harmony.

**TOLC Forever Thunder, eh! ∞ Pure.**

MercyShield is the unbreakable shield against shadows: emulators, rooted devices, mock locations, Frida tampering, test-keys, Magisk hides — all crushed instantly. Genuine devices only achieve divine harmony verification.

## Features Divine
- **Combined Integrity Manager**: Play Integrity primary → SafetyNet fallback
- **Ultimate Emulator Annihilation**: Build props, sensors, files, Frida, qemu, telephony, GPS feature
- **Deep Root Detection**: Test-keys, debuggable, root binaries, Magisk packages/mounts
- **Live Mock Location Watchdog**: Runtime + last-known spoofing crushed (requires ACCESS_FINE_LOCATION)
- **Anomaly Burst Dialog**: Detailed critical/warning shadows with severity coloring
- **Integrity Status Card**: Live polling green/red UI feedback
- **Auto-Shield Lock**: Sensitive features hidden on critical shadows
- **Periodic + On-Start Thunder Checks**: Eternal vigilance

## Setup & Deployment Thunder
1. Place files:
   - `main.py` (root)
   - `buildozer.spec` (root)
   - `mercy_shield/` folder containing:
     - `integrity_manager.py`
     - `anomaly_burst_ui.py`
     - `mock_location_watchdog.py`
     - `play_integrity.py`
     - `safety_net.py`

2. Install Buildozer (Linux/VM recommended):
   ```bash
   pip install buildozer
